...
func GetGithubReleasesLatest(repo string) string {
	...
	if resp.StatusCode != http.StatusOK {
		...
		fmt.Printf("GitHub API returned %d: %s\n", resp.StatusCode, string(body))
		return ""
	}
...
// GetGithubReleaseUrls returns URLs of all release assets whose FILENAME
// (not the full URL) matches the supplied regex pattern.
func GetGithubReleaseUrls(meta_data string, regex string) []string {
	...
	// Compile the regex once, with proper error handling.
	// If the pattern is invalid (e.g. broken YAML escaping upstream),
	...
	// Iterate over the assets and find all whose filename matches the regex
	...
	for _, asset := range assets {
		...
		// Match against just the filename, not the full URL, to avoid
		// accidental matches against the repo/org/path segments, and to
		// avoid unanchored substring matches against sibling files like
		// "jadx-1.5.6.zip.sha256".
		fileName := ExtractFileNameFromURL(browser_download_url)
...
func main() {
	if len(os.Args) != 4 {
		fmt.Println("Usage: go run githubdownload.go <repo> <regex> <output_directory>")
		fmt.Println("For Example: go run githubdownload.go 'NationalSecurityAgency/ghidra' 'ghidra_.*_PUBLIC_.*zip$' '.'")
		return
	}
	...
	tagFilePath := filepath.Join(output_directory, "tag_name.txt")

	// Safely check and read tag_name.txt only if it exists
	localTag := ""
	if _, err := os.Stat(tagFilePath); err == nil {
		localTag, _ = ReadFileAsString(tagFilePath)
	}

	releasePage := GetGithubReleasesLatest(repo)
	if releasePage == "" {
		fmt.Println("Error: Failed to fetch release page from GitHub.")
		return
	}

	remoteTag := GetGithubReleaseTagName(releasePage)

	if localTag != "" && localTag == remoteTag {
